...
import { club_request as DataModel } from '../Model';
...
class ClubReqs extends React.Component {
    constructor(props) {
        super();
        this.state = {
            data_list: [],
    ...
    onDeleteItem = (del_id) => {
        console.log(del_id);
        if (del_id) {
            const club_id = this.normalizeClubId(del_id)[0];
            const user_id = this.normalizeClubId(del_id)[1];
            console.log(club_id, user_id);
            adminApi
                .delClubReq(club_id, user_id)
                .then((response) => {
                    if (response.status === true) {
                        this.setState({
                        ...
                        helper.showToast(
                            'Success',
                            'Club Request deleted successfully',
                            1
                        );
                    } else {
                ...
                .catch((error) => {
                    ...
                    helper.showToast(
                        'Error',
                        error.message ||
                            'There was an error while deleting the club request.',
                        3
                    );
                });
        }
    ...
    onDeleteBulk = async (delIds: [String], callBack = () => {}) => {
        ...
        for (const del_id of delIds) {
            const club_id = this.normalizeClubId(del_id)[0];
            const user_id = this.normalizeClubId(del_id)[1];
            promiseList.push(adminApi.delClubReq(club_id, user_id));
        }
