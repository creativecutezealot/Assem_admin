...
import { audio as DataModel } from '../Model';
...
class Audios extends React.Component {
    ...
    handleChangeBoolVal = (event) => {
        ...
        console.log(obj);
        adminApi
            .updateClub(club_id, obj)
            .then((response) => {
                if (response.status === true) {
                    console.log(JSON.stringify(response.data));
                    var result = response.data;
                    const find_index = data_list.findIndex(
                        (r) => r.club_id === result.club_id
                    );
                    if (find_index !== -1) {
    ...
    onDeleteBulk = async (delIds, callBack = () => {}) => {
        ...
        for (const del_id of delIds) {
            promiseList.push(adminApi.deleteAudio(del_id));
        }
